...
//    bool selectivity = true;
    ...
    analysisIncrDetGraph analyzeGraph(graphGenerator.getGraph(), conf);
//    analyzeGraph.numberOfNodesAnalysis();
    

    ofstream rFile;
    rFile.open("distributionAnalysis.r", ios::trunc);
    ...
    for(int i=0; i<conf.schema.edges.size(); i++) {
    	...
    	analyzeGraph.distributionAnalysis(i, rFile);
    	rFile << endl;
    }
    rFile.close();

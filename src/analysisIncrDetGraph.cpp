...
void analysisIncrDetGraph::printToRfile(ofstream& rFile, bool outDistr, config::edge edge) {
	...
	if(distr->type == DISTRIBUTION::UNIFORM) {
		...
		rFile <<"zipfian <- c()" << endl;
		rFile << "maxValue = sort("<< distributionVar <<", decreasing = TRUE)[1]" << endl;
		rFile << "xRange = c(1:maxValue)" << endl;
		rFile << "for(i in 1:maxValue) {" << endl;
		rFile << " zipfian <- c(zipfian, i^(-" << distr->arg2 << ")); " << endl;
		rFile << "}" << endl;
		rFile << "par(pch=22, col=\"blue\");" << endl;
		rFile << "lines(xRange-1, zipfian, type=\"l\", lwd=3);" << endl;
